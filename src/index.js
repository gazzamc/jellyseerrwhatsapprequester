...
const { SESSION_PATH, ENABLE_EVENT_MESSAGES, CHAT_WHITELIST } = require('./config')
const { requestMedia, isRequested, searchJellyseerr, processCustomMessage, buildResponse } = require('./utils')
    ...
    if (ENABLE_EVENT_MESSAGES) {
        ...
        for (let i = 0; i < chats.length; i++) {
            if (CHAT_WHITELIST.includes(chats[i].name.toLowerCase())) {
                chats[i].sendMessage(processCustomMessage('BOT_READY', [usage]) || 'Bot Ready')
            }
        }
    }
});
...
client.on('message', async msg => {
    ...
    if (pendingSelections[senderId] && /^[0-9]+$/.test(text)) {
        ...
        if (selectionIndex >= 0 && selectionIndex < session.results.length) {
            ...
            await msg.reply(processCustomMessage('INVALID_SEL') || `⚠️ Invalid selection. Please enter a number from the list.`);
        }
        return;
    }

    // Command parsing
    if (text.toLowerCase().startsWith('!request') || text.toLowerCase().startsWith('!r')) {
        let isDefault = true;
        let type = 'movie'; // default
        let searchTerm = text.toLowerCase().startsWith('!request') ? text.replace(/^!request\s*/i, '') : text.replace(/^!r\s*/i, '')
        ...
        if (!searchTerm.trim()) {
            await msg.reply(processCustomMessage('NO_TERM') || `⚠️ Please provide a search term. Example:\n!request movie ironman`);
            return;
        }
        ...
        try {
            ...
            if (results.length === 0) {
                const response = isDefault ? `No type provided, used movie (default) and found nothing for "${searchTerm}"` : `No ${type === 'movie' ? 'movies' : 'series'} found for "${searchTerm}"`;
                await msg.reply(processCustomMessage('REQ_NO_ITEM', [type, searchTerm, isDefault]) || response);
                return;
            }
    ...
    // Add help command
    if (text.toLowerCase().startsWith('!help') || text.toLowerCase().startsWith('!h')) {
